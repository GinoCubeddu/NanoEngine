    ...
    static class PhysicsMethods
    {
        public static void Bounce(IAsset asset1, IAsset asset2, Tuple<NanoCollisionEventArgs, NanoCollisionEventArgs> eventArgs)
        {
            if (asset1 is IBounce && asset2 is IBounce)
            {
                ...
                Vector2 directionAsset1 = Vector2.Normalize(-eventArgs.Item1.CollisionOverlap);
                Vector2 directionAsset2 = Vector2.Normalize(-eventArgs.Item2.CollisionOverlap);
                // Console.WriteLine("cn "+ cn);

                //if (asset1.Position.X < asset2.Position.X || asset1.Position.Y < asset2.Position.Y)
                  //  cn = -cn;
            ...
            {
                asset1.SetPosition(asset1.Position - eventArgs.Item1.CollisionOverlap);
                Vector2 overlap = ((PhysicsEntity)asset1).Position - (asset2.Position);
            ...
            } else if (asset2 is IBounce)
            {
                ...
                asset2.SetPosition(asset2.Position - eventArgs.Item2.CollisionOverlap);
                Vector2 overlap = (asset1.Position) - ((PhysicsEntity)asset2).Position;
              ...
              //  Vector2 velocityA = -(cn * cvB);
                ...
                //reflect off plane
                // ((PhysicsEntity)asset2).ApplyImpluse(((PhysicsEntity)asset2).Velocity * -2);
            }
